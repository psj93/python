...
#         떡을 사갈 수 있습니다. 높이는 10억보다 작거나 같은 양의 정수 또는 0입니다.
...
#'현재 이 높이로 자르면 조건을 만족할 수 있는가?'를 확인한 뒤에 조건의 만족 여부('예' 혹은 '아니오')에 따라서
#탐색 범위를 좁혀서 해결할 수 있음
#절단기 높이는 0부터 10억까지의 정수 중 하나임
# 이렇게 큰 탐색 범위를 보면 가장 먼저 이진 탐색을 떠올려야 함
#문제에서 제시된 예시를 통해 그림으로 이해해 봄

#step1.시작점:0, 끝점:19, 중간점:9 <- 이때 필요한 떡의 크기 M = 6이므로, 결과 저장
#      잘린 떡의 길이:10, 6, 1, 8 (25)
#step2.시작점:10, 끝점:19, 중간점:14 <- 이때 필요한 떡의 크기 M = 6이므로, 결과 저장
#      잘린 떡의 길이:5, 1, 0, 3 (9)
#step3.시작점:15, 끝점:19, 중간점:17 <- 이때 필요한 떡의 크기 M = 6이므로, 결과 저장하지 않음
#      잘린 떡의 길이:2, 0, 0, 0 (2)
#step4.시작점:15, 끝점:16, 중간점:15 <- 이때 필요한 떡의 크기 M = 6이므로, 결과 저장
#      잘린 떡의 길이:4, 0, 0, 2 (6)
...
#예를 들어 수열 {1, 1, 2, 2, 2, 2, 3}이 있을 때 x = 2 라면,
#현재 수열에서 값이 2인 원소가 4개 이므로 4를 출력함
#단, 이 문제는 시간 복잡도 O(logN)으로 알고리즘을 설계하지 않으면 시간 초과 판정을 받음

<문제>정렬된 배열에서 특정 수의 개수 구하기:문제 조건
#입력 조건:첫째 줄에 N과 x가 정수 형태로 공백으로 구분되어 입력됨 (1 <= N <= 1,000,000), (-10^9 <= x <= 10^9)
#         둘째 줄에 N개의 원소가 정수 형태로 공백으로 구분되어 입력됨 (-10^9 <= 각 원소의 값 <= 10^9)
#출력 조건:수열의 원소 중에서 값이 x인 원소의 개수를 출력함. 단, 값이 x인 원소가 하나도 없다면 -1을 출력함
#입력 예시: 7 2
#          1 1 2 2 2 2 3
#출력 예시: 4
...
n, x = map(int, input().split()) #데이터의 개수 N, 찾고자 하는 값 x 입력하기
