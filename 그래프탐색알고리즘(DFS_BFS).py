...
#미로에는 여려 마리의 괴물이 있어 이를 피해 탈출해야 합니다.
#동빈이의 위치는 (1,1)이며 미로의 출구는 (N,N)의 위치에 존재하며 한 번에 한 칸씩 이동할 수 있습니다.
...
<문제>미로 탈출:문제 조건
#입력 조건:첫째 줄에 두 정수 N, M(4 <= N, M <= 200)이 주어집니다. 다음 N개의 줄에는 각각 M개의 정수(0 혹은 1)로
          미로의 정보가 주어집니다. 각각의 수들은 공백 없이 붙어서 입력으로 제시됩니다. 또한 시작 칸과 마지막 칸은
          항상 1입니다.
...
# 따라서 (1,0) 지점부터 BFS를 수행하여 모든 노드의 최단ㄷ 거리 값을 기록하면 해결할 수 있음

#step1.처음에 (1,1)의 위치에서 시작함
#step2.(1,1) 좌표에서 상,하,좌,우로 탐색을 진행하면 바로 옆 노드인 (1,2) 위치의 노드를 방문하게 되고
#      새롭게 방문하는 (1,2) 노드의 값을 2로 바꾸게 됨
